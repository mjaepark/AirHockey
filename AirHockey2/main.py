...
def score(score1, score2, player_1_name, player_2_name):
    text1 = smallfont.render("{0} : {1}".format(player_1_name, str(score1)), True, const.WHITE)
    text2 = smallfont.render("{0} : {1}".format(player_2_name, str(score2)), True, const.WHITE)

    screen.blit(text1, [40, 0])
    screen.blit(text2, [width - 200, 0])


def rounds(rounds_p1, rounds_p2, round_no):
    disp_text(screen, "Round "+str(round_no), (width/2, 20), roundfont, const.WHITE)
    disp_text(screen, str(rounds_p1) + " : " + str(rounds_p2), (width / 2, 50), roundfont, const.WHITE)
...
def notify_round_change():
    while True:
        ...
        score_text = roundfont.render("{0}  :  {1}".format(score1, score2), True, const.WHITE)
...
def show_pause_screen():
    ...
    while True:
        text_pause = smallfont.render("PAUSED", True, const.WHITE)
        screen.blit(text_pause, [width / 2 - 74, 230])
...
def render_field(background_img):
    # Render Logic
...
# Game Loop
def game_loop(speed, player1_color, player2_color, background_img, background_color, player_1_name, player_2_name, mod):
    ...
    while True:
        ...
        p1_x_change = p1_y_change = p2_x_change = p2_y_change = 0
        w = s = a = d = False
        ...
        if result.multi_hand_landmarks:
            p1_x, p1_y = paddle1.get_pos()
            p2_x, p2_y = paddle2.get_pos()
        ...
        if p2_y_change < -15:
            ...
            pass
        show = cv2.resize(frame, (340, 170), interpolation = cv2.INTER_AREA)
        show = cv2.flip(show, 1)
        show = cv2.line(show, (170, 0), (170, 170), const.BLACK, 5)
        cv2.namedWindow("Video")
        cv2.moveWindow("Video", 1100, 650)
        ...
        if goal:
            count = ["3", "2", "1", "Start!!"]
            count_font = pygame.font.SysFont("arial", 100, True, True)
            c = 0
            for k in count:
                render_field(background_img)
                score(score1, score2, player_1_name, player_2_name)
                count_text = count_font.render(k, True, theme_colors[c][0])
                screen.blit(count_text, (550, 250))
        ...
        if rounds_p1 == const.ROUND_LIMIT:  # Player one denotes left player
            if end(game_end(screen, clock, background_img, background_color, player_1_name), speed):
                if music_paused:
                    pygame.mixer.music.unpause()
                pygame.mixer.stop()
                return
        elif rounds_p2 == const.ROUND_LIMIT:  # Player two denotes right player
            if end(game_end(screen, clock, background_img, background_color, player_2_name), speed):
                if music_paused:
                    pygame.mixer.music.unpause()
                pygame.mixer.stop()
                return
        ...
        if mod == 2:
            paddle1.paddleChange()
            paddle2.paddleChange()
            paddle1.draw_crazy(screen, player1_color)
            paddle2.draw_crazy(screen, player2_color)
            puck.puckChange()
            puck.draw_crazy(screen)
        else:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        background_img, background_color = theme_screen(screen, clock, width, height, mute)
        init()
        if gameChoice == 1:
            puck.speed = const.EASY
            game_loop(const.EASY, player1_color, player2_color, background_img, background_color, player_1_name, player_2_name, gameChoice)
        elif gameChoice == 2:
            puck.speed = const.HARD
            game_loop(const.HARD, player1_color, player2_color, background_img, background_color, player_1_name, player_2_name, gameChoice)
        elif gameChoice == 0:
            sys.exit()